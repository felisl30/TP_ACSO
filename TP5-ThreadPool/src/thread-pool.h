...
typedef struct worker {
    thread ts;
    function<void(void)> thunk = nullptr;
    /**
     * Complete the definition of the worker_t struct here...
     **/
    condition_variable workerCV;    // Para despertar al worker
    bool available = true;          // Estado de disponibilidad del worker
  ...
  */
  ...
  private:
    ...
    */
    ...
    Semaphore dispatcherSemaphore{0};       // semáforo para despertar al dispatcher
    Semaphore workerAvailableSemaphore;     // semáforo para workers disponibles
    // queue<function<void(void)>> tasks;           // cola de tareas
    condition_variable dispatcherCV;             // CV para despertar al dispatcher  
    condition_variable waitCV;                   // CV para la función wait()
    int activeWorkers = 0; 
